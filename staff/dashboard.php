...
$output="";
    ...
    if ($filter==2) {
      $check_user = mysqli_query($conn, "SELECT * FROM proposals INNER JOIN date_of_proposals ON (proposals.proposal_id=date_of_proposals.proposal_id) WHERE proposals.status='Draft' AND proposals.session='$session' ORDER BY proposal_date DESC, product_name ASC");//session
    }elseif ($filter==3) {
      $check_user = mysqli_query($conn, "SELECT * FROM proposals INNER JOIN date_of_proposals ON (proposals.proposal_id=date_of_proposals.proposal_id) WHERE proposals.status='Ok for Sending' AND proposals.session='$session' ORDER BY proposal_date DESC, product_name ASC");//session
    }elseif ($filter==4) {
      $check_user = mysqli_query($conn, "SELECT * FROM proposals INNER JOIN date_of_proposals ON (proposals.proposal_id=date_of_proposals.proposal_id) WHERE proposals.status='For Approval' AND proposals.session='$session' ORDER BY proposal_date DESC, product_name ASC");//session
    }else{
      $check_user = mysqli_query($conn, "SELECT * FROM proposals INNER JOIN date_of_proposals ON (proposals.proposal_id=date_of_proposals.proposal_id) WHERE proposals.status!='dummy' AND proposals.session='$session' ORDER BY proposal_date DESC, product_name ASC");//session
    }
